...
export const getSortedCampagnes = async (isAdmin, isObserver, userSiret = [], sortingType, scope) => {
  try {
    let campagnes = [];
    const etablissementsFromReferentiel = await referentiel.getEtablissements(userSiret);
...
export const createCampagne = async (campagne) => {
  try {
...
export const createMultiCampagne = async (campagnes, currentUserId) => {
  try {
    const formationsIds = [];
